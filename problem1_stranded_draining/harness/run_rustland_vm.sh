...
GATE_FIFO="${OUT_DIR}/workload.gate"
...
sleep "${GATE_WRITE_DELAY_SEC:-0.05}"
if ! timeout "${GATE_WRITE_TIMEOUT_SEC:-2}" bash -c 'printf x > "$1"' _ "${GATE_FIFO}"; then
  echo "failed to release workload gate ${GATE_FIFO}" >>"${WORKLOAD_STDERR}"
fi
